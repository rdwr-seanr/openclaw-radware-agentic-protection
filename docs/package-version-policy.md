...
The legacy unscoped package is `openclaw-radware-agentic-protection`. Keep it available as a migration pointer; do not delete it as a normal maintenance action. Deprecate it only from an npm account that owns that package.
...
Deprecate the old unscoped package with a migration message:
...
The Radware scoped-account token cannot deprecate the legacy unscoped package unless that account is added as a maintainer of the old package.
